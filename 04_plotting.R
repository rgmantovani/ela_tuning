...
# Requires: 03_shap_analysis.py must be run first (generates tmp/shap_*.csv)
...
dir.create("./plots/hyperspaces", showWarnings = FALSE, recursive = TRUE)
dir.create("./plots/analysis",    showWarnings = FALSE, recursive = TRUE)
...
for (file in land_files) {
  ...
  g <- ggplot(data, aes(x = cost, y = gamma, colour = BAC)) +
    geom_point(size = 0.1) +
    scale_colour_gradient2(low = "red", high = "blue", mid = "white", midpoint = 0.5) +
    labs(x = "log2(cost)", y = "log2(gamma)")

  out_file <- paste0("./plots/hyperspaces/",
    gsub(x = file, pattern = ".csv", replacement = "_hyperspace.pdf", fixed = TRUE))
  ggsave(g, file = out_file, width = 3.49, height = 2.56)
  cat("Saved:", out_file, "\n")
}

# ---------------------------------
# Load and aggregate meta-learning results
# ---------------------------------

result_files <- list.files("results", pattern = "loo_metrics_seed_", full.names = TRUE)
all_data <- do.call(rbind, lapply(result_files, read.csv))
...
ds_levels  <- c("Baseline", "Ela_metadataset", "Combined")
ds_labels  <- c("Baseline", "ELA Meta-dataset", "Combined")

all_data$algorithm <- factor(all_data$algorithm, levels = alg_order,  labels = alg_labels)
all_data$dataset   <- factor(all_data$dataset,   levels = ds_levels,  labels = ds_labels)

# ---------------------------------
# Plot 1 — Violin + boxplot: AUC by dataset
# ---------------------------------
...
# ---------------------------------
# Plot 2 — Paired dot plot: ELA / Combined vs Baseline
# ---------------------------------
...
alg_shapes  <- c(DT = 15, KNN = 16, LR = 17, NB = 18, RF = 8,
                 "SVM-Lin" = 10, "SVM-RBF" = 11, XGBoost = 12)
alg_colours <- setNames(brewer.pal(8, "Dark2"), levels(agg_wide$algorithm))

agg_long <- rbind(
  data.frame(agg_wide[, c("algorithm", "threshold", "baseline")],
             comparison = "ELA Meta-dataset vs Baseline", y = agg_wide$ela),
  data.frame(agg_wide[, c("algorithm", "threshold", "baseline")],
             comparison = "Combined vs Baseline",         y = agg_wide$combined)
)
...
# ---------------------------------
# Plot 4 — Heatmap: algorithm × threshold, faceted by dataset
# ---------------------------------
...
best_alg <- do.call(rbind, lapply(split(best_alg, best_alg$dataset), function(d) d[which.max(d$AUC), ]))
agg3$is_best <- paste(agg3$algorithm, agg3$dataset) %in% paste(best_alg$algorithm, best_alg$dataset)
...
# ---------------------------------
# SHAP plots (requires 04_shap_analysis.py to have been run first)
# ---------------------------------

make_shap_plots <- function(tag, out_file) {
  ...
  feat_levels <- top10_names

  # Beeswarm
  shap_long <- do.call(rbind, lapply(feat_levels, function(f) {
  ...
  # Bar
  bar_df <- tail(data.frame(feature    = names(mean_shap),
                             importance = as.numeric(mean_shap))[order(as.numeric(mean_shap)), ], 10)
  ...
  ggsave(file.path(OUT, out_file),
         g_bee + g_bar + plot_layout(widths = c(2, 1)),
         width = 12, height = 5)
  cat(out_file, "saved\n")
}
